...
export class VoxelWorld {
  ...
  // deprecated - use buildCulledMesh() instead
  /*
  createVoxel(x, y, z) {
  ...
  */
  ...
  buildCulledMesh() {
    // A quick geometry + material approach
    // In a real voxel engine, you might do advanced UVs, atlas lookups, merges, etc.
    const positions = [];
    const normals   = [];
    const uvs       = [];
    const indices   = [];
    
    // We’ll track the current vertex index while building faces:
    let currentIndex = 0;
    
    // Helper for pushing a face (square) of data
    const pushFace = (facePositions, faceNormals, faceUVs) => {
      // Each face has 4 vertices. We'll add them (positions, normals, uvs) and then 2 triangles
      for (let i = 0; i < 4; i++) {
    ...
    // Offsets for the 6 directions, plus the 4 corners in correct CCW order:
    const directions = {
      px: { // +X face
        neighbor: [ 1,  0,  0],
        positions: [
        ...
        normal: [1, 0, 0],
        uv: [[0,1],[0,0],[1,0],[1,1]] // You can reorder these as needed
      },
      nx: { // -X face
        neighbor: [-1,  0,  0],
        positions: [
    ...
    // For each block in worldData, check neighbors
    for (let key in this.worldData) {
      if (!this.worldData.hasOwnProperty(key)) continue;
      
      const [x, y, z] = key.split(',').map(Number);

      // For each of the 6 directions, add the face if neighbor is missing
      for (let dirKey in directions) {
        ...
        //const fullPath = /* reconstruct or store a direct mapping */ 
        const texture = this.textureManager.getTexture(textureName);
        console.log(texture);
        const atlasUV = texture;
        console.log(atlasUV.offset.x);
        if (!atlasUV) continue; // fallback?

        // 3. The 'dir.uv' array is basically the shape of the face in [0..1] space. 
        //    We must shift it by atlasUV.offset and scale it by atlasUV.size:
        const faceUVs = dir.uv.map( uv => {
          const u = atlasUV.offset.x;
          const v = atlasUV.offset.y;
          return [u, v];
        });

        // 4. Now we push the final face data
        pushFace(facePositions, faceNormals, faceUVs);
          
      }
    }

    // Build a single BufferGeometry from the face data
    ...
    // Use any material you like. If you have an atlas, you can apply it here:
